...
import { AnalysisItem, ChatMessage } from '../types';
...
const InterviewChat: React.FC<InterviewChatProps> = ({ item, onFinish, onCancel }) => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const responseText = await chatWithInterviewer(currentHistory, item);
  ...
  return (
    <div className="max-w-3xl mx-auto h-[600px] flex flex-col bg-slate-900 border border-slate-700 rounded-xl overflow-hidden shadow-2xl animate-fade-in-up relative">
      
      {/* Concluding Overlay */}
      {isConcluding && (
        <div className="absolute inset-0 z-50 bg-slate-900/90 backdrop-blur-sm flex flex-col items-center justify-center p-6 animate-fade-in text-center">
          ...
          <h3 className="text-2xl font-bold text-white mb-2">면접 방어 성공!</h3>
          <p className="text-slate-300 mb-6">충분히 설명되었습니다. 잠시 후 피드백 페이지로 이동합니다.</p>
      ...
      <div className="bg-slate-800 p-4 border-b border-slate-700 flex justify-between items-center z-10">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">주제: {item.topic}</p>
        </div>
        <button 
          onClick={onCancel}
          disabled={isConcluding}
          className="text-slate-400 hover:text-white text-sm px-3 py-1 rounded hover:bg-slate-700 transition-colors disabled:opacity-50"
        >
          나가기
        </button>
      </div>
      ...
      <div className="p-4 bg-slate-800 border-t border-slate-700 z-10">
        <form onSubmit={handleSendMessage} className="flex gap-2">
          <input
            ...
            placeholder={isConcluding ? "면접이 종료되었습니다." : "답변을 입력하세요..."}
            disabled={isTyping || isConcluding}
            className="flex-1 bg-slate-900 border border-slate-600 rounded-lg px-4 py-3 text-slate-200 focus:ring-2 focus:ring-rose-500 focus:outline-none disabled:bg-slate-950 disabled:text-slate-600"
            autoFocus
          />
          <button
            type="submit"
            disabled={!inputValue.trim() || isTyping || isConcluding}
            className="bg-rose-600 hover:bg-rose-700 disabled:bg-slate-700 disabled:text-slate-500 text-white px-6 py-3 rounded-lg font-bold transition-colors"
          >
            전송
          </button>
        </form>
        {!isConcluding && (
          <div className="mt-3 flex justify-center">
             <button 
               ...
               className="text-xs text-slate-400 hover:text-rose-400 underline decoration-dotted transition-colors"
             >
